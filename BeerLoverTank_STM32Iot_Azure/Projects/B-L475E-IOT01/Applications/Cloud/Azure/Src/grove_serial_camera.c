 ...
 * skipFrame: the number of dropped frames can be defined before compression occurs. g0h keeps the current frame, g1h captures the next frame ect.
 */
 ...
 * Verify Code: Error  detection  code,  equals  to  the  lower  byte  of  sum  of  the  whole  package  data  except  the  verify   code   field.   The   higher   byte   of   this   code   is   always   zero.   i.e.   verify   code=low   byte(sum(bytey0zto byteyN-2z)        Note:  As  the  transmission  of  uncompressed  image  is  not  the  package  mode,  it  is  not  necessary  to  set  the  package size for the uncompressed image.
 */
static SERIAL_CAM_Error _cmd_setPackageSize(uint8_t packageSizeL, uint8_t packageSizeH) {
...
/**
 * This  command  indicates  the  success  of  the  last  operation.  After  receiving  any  valid  command,  ACK  command  must  be  sent  out  except  when  getting  preview  data.      The  host  can  issue  this  command  request  image  data  package  with  desired  package  ID  after  receiving  Data  command  from  camera,  and  send  this  command with package ID 0F0Fh when finishing with receiving data package.     Note: gCommand IDh is 00h when host requesting image data package.
